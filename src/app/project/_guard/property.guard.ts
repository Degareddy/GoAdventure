...
export class PropertyGuard implements CanActivate {
  ...
  //     const propertyItems = menuData.filter((x: any) => x.menuTitle === 'Property');
  //     if (propertyItems.length > 0) {
  //       const subUrls = propertyItems
  //         .filter((item: any) => item.menuTitle === 'Property')
  //         .map((item: any) => item.subUrl.replace('/property/', ''));
  ...
  //           TENANT: subUrls
  //         };
  //         if (roleAccessMap[userRole]?.includes(routePath.replace('/property/', ''))) {
  //           return true;
  //         }
  //         return false;
  //     }
  ...
  canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): boolean {
    ...
    const propertyItems = menuData.filter((item: any) => item.menuTitle === 'Property');

    if (propertyItems.length === 0) {
      return false;
    }

    const subUrls = propertyItems.map((item: any) => item.subUrl.replace('/property/', ''));

    const userRole = this.userServie.userData.userProfile.toUpperCase();

    const routePath = state.url.replace('/property/', '');
