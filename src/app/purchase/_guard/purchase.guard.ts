...
//   constructor(private userServie: UserDataService, private router: Router) {}
//   canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): boolean {
//     const menu = sessionStorage.getItem('menuData');
//     if (menu) {
//       const menuData = JSON.parse(menu);
//       const propertyItems = menuData.filter((x: any) => x.menuTitle === 'Purchase');
//       if (propertyItems.length > 0) {
//         const subUrls = propertyItems
//           .filter((item: any) => item.menuTitle === 'Purchase')
//           .map((item: any) => item.subUrl.replace('/purchase/', ''));
...
//             TENANT: subUrls
//           };
//           if (roleAccessMap[userRole]?.includes(routePath.replace('/purchase/', ''))) {
//             return true;
//           }
//           return false;
//       }
...
export class PurchaseGuard implements CanActivate {
  ...
  //         .filter((item: any) => item.menuTitle === 'Purchase')
  //         .map((item: any) => {
  //           return item.subUrl.replace(/^\/?purchase\//, '');
  //         });

  //       const userRole = this.userServie.userData.userProfile.toUpperCase();
  //       const routePath = state.url.replace(/^\/?purchase\//, '');
  //       const roleAccessMap: { [role: string]: string[] } = {
  //         CMPADMIN: subUrls,
  //         CMPUSER: subUrls,
  //         GADMIN: subUrls,
  ...
  canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): boolean {
    ...
    const purchaseItems = menuData.filter((item: any) => item.menuTitle === 'Purchase');
    if (purchaseItems.length === 0) {
      return false;
    }

    const subUrls = purchaseItems.map((item: any) => item.subUrl.replace(/^\/?purchase\//, ''));
