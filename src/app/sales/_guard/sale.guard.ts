...
export class SaleGuard implements CanActivate {
  ...
  //     const propertyItems = menuData.filter((x: any) => x.menuTitle === 'Sales');
  //     if (propertyItems.length > 0) {
  //       const subUrls = propertyItems
  //         .filter((item: any) => item.menuTitle === 'Sales')
  //         .map((item: any) => item.subUrl.replace('/sales/', ''));
  ...
  //           TENANT: subUrls
  //         };
  //         if (roleAccessMap[userRole]?.includes(routePath.replace('/sales/', ''))) {
  //           return true;
  //         }
  //         return false;
  //     }
  ...
  canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): boolean {
    ...
    const salesItems = menuData.filter((item: any) => item.menuTitle === 'Sales');
    if (salesItems.length === 0) {
      return false;
    }

    const subUrls = salesItems.map((item: any) => item.subUrl.replace('/sales/', ''));

    const userRole = this.userServie.userData.userProfile.toUpperCase();

    const routePath = state.url.replace('/sales/', '');
