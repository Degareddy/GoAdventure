...
export class GlGuard implements CanActivate {
  ...
  //     const propertyItems = menuData.filter((x: any) => x.menuTitle === 'General Ledger');
  //     if (propertyItems.length > 0) {
  //       const subUrls = propertyItems
  //         .filter((item: any) => item.menuTitle === 'General Ledger')
  //         .map((item: any) => item.subUrl.replace('/gl/', ''));
  ...
  //           TENANT: subUrls
  //         };
  //         if (roleAccessMap[userRole]?.includes(routePath.replace('/gl/', ''))) {
  //           return true;
  //         }
  //         return false;
  //     }
  ...
  canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): boolean {
    ...
    const glItems = menuData.filter((item: any) => item.menuTitle === 'General Ledger');
    if (glItems.length === 0) {
      return false;
    }

    const subUrls = glItems.map((item: any) => item.subUrl.replace('/gl/', ''));

    const userRole = this.userServie.userData.userProfile.toUpperCase();

    const routePath = state.url.replace('/gl/', '');
