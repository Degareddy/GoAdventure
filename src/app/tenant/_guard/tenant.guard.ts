...
export class TenantGuard implements CanActivate {
  ...
  //       const propertyItems = menuData.filter((x: any) => x.menuTitle === 'Tenant');
  //       if (propertyItems.length > 0) {
  //         const subUrls = propertyItems
  //           .filter((item: any) => item.menuTitle === 'Tenant')
  //           .map((item: any) => item.subUrl.replace('/tenant/', ''));
  ...
  //           TENANT: subUrls
  //         };
  //           if (roleAccessMap[userRole]?.includes(routePath.replace('/tenant/', ''))) {
  //             return true;
  //           }
  //           return false;
  //       }
  ...
  canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): boolean {
    ...
    const tenantItems = menuData.filter((item: any) => item.menuTitle === 'Tenant');
    if (tenantItems.length === 0) {
      return false;
    }

    const subUrls = tenantItems.map((item: any) => item.subUrl.replace(/^\/?tenant\//, ''));

    const userRole = this.userServie.userData.userProfile.toUpperCase();

    const routePath = state.url.replace(/^\/?tenant\//, '');
