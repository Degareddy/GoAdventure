...
export class PayrollGuard implements CanActivate {
  ...
  //     const propertyItems = menuData.filter((x: any) => x.menuTitle === 'Payroll');
  //     if (propertyItems.length > 0) {
  //       const subUrls = propertyItems
  //         .filter((item: any) => item.menuTitle === 'Payroll')
  //         .map((item: any) => item.subUrl.replace('/payroll/', ''));
  ...
  //           TENANT: subUrls
  //         };
  //         if (roleAccessMap[userRole]?.includes(routePath.replace('/payroll/', ''))) {
  //           return true;
  //         }
  //         return false;
  //     }
  ...
  canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): boolean {
    ...
    const payrollItems = menuData.filter((item: any) => item.menuTitle === 'Payroll');
    if (payrollItems.length === 0) {
      return false;
    }

    const subUrls = payrollItems.map((item: any) => item.subUrl.replace('/payroll/', ''));

    const userRole = this.userServie.userData.userProfile.toUpperCase();

    const routePath = state.url.replace('/payroll/', '');
