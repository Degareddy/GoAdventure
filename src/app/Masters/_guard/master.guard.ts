...
export class MasterGuard implements CanActivate {
  ...
  //     const propertyItems = menuData.filter((x: any) => x.menuTitle === 'Master');
  //     if (propertyItems.length > 0) {
  //       const subUrls = propertyItems
  //         .filter((item: any) => item.menuTitle === 'Master')
  //         .map((item: any) => item.subUrl.replace('/master/', ''));
  ...
  //           TENANT: subUrls
  //         };
  //         if (roleAccessMap[userRole]?.includes(routePath.replace('/master/', ''))) {
  //           return true;
  //         }
  //         return false;
  //     }
  ...
  canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): boolean {
    ...
    const masterItems = menuData.filter((item: any) => item.menuTitle === 'Master');
    if (masterItems.length === 0) {
      return false;
    }

    const subUrls = masterItems.map((item: any) => item.subUrl.replace('/master/', ''));

    const userRole = this.userServie.userData.userProfile.toUpperCase();

    const routePath = state.url.replace('/master/', '');
