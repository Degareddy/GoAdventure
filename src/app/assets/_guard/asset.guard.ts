...
export class AssetGuard implements CanActivate {
  ...
  //     const propertyItems = menuData.filter((x: any) => x.menuTitle === 'Assets');
  //     if (propertyItems.length > 0) {
  //       const subUrls = propertyItems
  //         .filter((item: any) => item.menuTitle === 'Assets')
  //         .map((item: any) => item.subUrl.replace('/assets/', ''));
  ...
  //           TENANT: subUrls
  //         };
  //         if (roleAccessMap[userRole]?.includes(routePath.replace('/assets/', ''))) {
  //           return true;
  //         }
  //         return false;
  //     }
  ...
  canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): boolean {
    ...
    const assetItems = menuData.filter((item: any) => item.menuTitle === 'Assets');
    if (assetItems.length === 0) {
      return false;
    }

    const subUrls = assetItems.map((item: any) => item.subUrl.replace('/assets/', ''));

    const userRole = this.userServie.userData.userProfile.toUpperCase();

    const routePath = state.url.replace('/assets/', '');
