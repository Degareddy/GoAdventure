...
export class TransferDetailsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.getStockDetails(this.data.tranNo, this.trfDetForm.get('mode')?.value);
  }
  ...
  getStockDetails(tarnNo: string, mode: string) {
    const body = {
      ...this.commonParams(),
      TranNo: tarnNo,
    ...
    try {
      this.subSink.sink = this.invService.GetStockTransferDetails(body).subscribe((res: any) => {
    ...
    if (this.trfDetForm.invalid) {
      ...
      this.subSink.sink = this.invService.UpdateStockTransferDetails(this.stkTrfDtCls).subscribe((res: SaveApiResponse) => {
        if (res.status.toUpperCase() === "SUCCESS") {
          ...
          this.textMessageClass = "green";
          this.getStockDetails(res.tranNoNew, this.trfDetForm.get('mode')?.value);
        }
        else {
          this.retMessage = res.message;
          this.textMessageClass = "red";
